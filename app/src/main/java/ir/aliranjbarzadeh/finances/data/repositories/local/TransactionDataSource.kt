...
@Singleton
class TransactionDataSource @Inject constructor(private val dao: TransactionDao) : TransactionDataSource {
	override suspend fun list(): List<Transaction> {
		val items = dao.list();
		return items.map { it.toDomain() }
	}

	override suspend fun store(transaction: Transaction): Long = dao.store(TransactionModel.fromModel(transaction))
